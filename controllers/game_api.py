...
class GameApi:
    ...
    def setActiveDeck(self, context):
        req = '{0}setActiveDeck&deck_id={1}{2}'.format(self.config.apiUrl, context.deck, self.__getUrlAuthSection(context))
        f = urllib.urlopen(req)
        resp = f.read().decode('utf-8')
        return json.loads(resp)

    def startPracticeVsMe(self, context):
        req = '{0}startPracticeBattle&target_user_id={1}{2}'.format(self.config.apiUrl, context.userId, self.__getUrlAuthSection(context))
        f = urllib.urlopen(req)
        resp = f.read()
    ...
    def startAdventureMission(self, context, level_id):
        req = '{0}startMission&mission_id={1}{2}'.format(self.config.apiUrl,level_id, self.__getUrlAuthSection(context))
        f = urllib.urlopen(req)
        s = f.read().decode('utf-8')
        return json.loads(s)

    def finishAdventureBattle(self, context, battle_id):
        req = '{0}playCard&skip=true&host_id={1}&battle_id={2}{3}'.format(self.config.apiUrl, context.userId, battle_id, self.__getUrlAuthSection(context))
        f = urllib.urlopen(req)
        s = f.read().decode('utf-8')
        return json.loads(s)
    ...
    def updateInitFile(self, context):
        req = "{0}{1}&api_stat_time=249{2}".format(self.config.apiUrl, self.config.initMessageKey, self.__getUrlAuthSection(context))
    ...
    def getGuildScoresIndiv(self, context, war):
        req = "{0}getRankings&ranking_id=event_guild&ranking_index={1}{2}".format(self.config.apiUrl, str(50000+war), self.__getUrlAuthSection(context))
        f = urllib.urlopen(req)
        response = f.read().decode('utf-8')
        return json.loads(response)

    def getLeftPullTab(self, context):
        req = "{0}getRankings&ranking_id=event_guild_war&ranking_index=0{1}".format(self.config.apiUrl, self.__getUrlAuthSection(context))
        f = urllib.urlopen(req)
        response = f.read().decode('utf-8')
        return json.loads(response)

    def getRightPullTab(self, context):
        req = "{0}getRankings&ranking_id=event_guild_war&ranking_index=1{1}".format(self.config.apiUrl, self.__getUrlAuthSection(context))
        f = urllib.urlopen(req)
        response = f.read().decode('utf-8')
        return json.loads(response)
    ...
    def getSiegeLeftTab(self, context):
        req = "{0}getRankings&ranking_id=event_guild_siege&ranking_index=0{1}".format(self.config.apiUrl, self.__getUrlAuthSection(context))
        f = urllib.urlopen(req)
        s = f.read().decode('utf-8')
        return json.loads(s)

    def getSiegeRightTab(self, context):
        req = "{0}getRankings&ranking_id=event_guild_siege&ranking_index=1{1}".format(self.config.apiUrl, self.__getUrlAuthSection(context))
        f = urllib.urlopen(req)
        s = f.read().decode('utf-8')
        return json.loads(s)
